...
int polygon() {
    ...
    printf("Row number represent the scale of child polygon\n");
    printf("Column number represent the child polygon start point\n");
    printf("Matrix of dynamic program weight is :\n");
    for (int i=0;i < (EDGE_NUM + 1) * 3;i++) {
        putchar('-');
    }
    putchar('\n');
    for (int scale = 2; scale < EDGE_NUM; scale++) {
        printf("| ");
        for (int start = 0; start < EDGE_NUM - 2; start++) {
            if (start < EDGE_NUM - scale) {
                printf("%3d ",weight[scale][start]);
            } else {
                printf("    ");
            }
        }
